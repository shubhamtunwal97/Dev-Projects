...
object GPUImageFilterTools {
    fun showDialog(
        ...
        val filters = FilterList().apply {
            ...
            addFilter("Auto White Balance", FilterType.KUWAHARA)
            addFilter("Curve Fitting", FilterType.RGB_DILATION)
    ...
    private fun createFilterForType(context: Context, type: FilterType): GPUImageFilter {
        return when (type) {
            ...
            FilterType.LOOKUP_AMATORKA -> GPUImageLookupFilter().apply {
                bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.abc_ic_ab_back_material)
            }
            ...
            FilterType.ZOOM_BLUR -> GPUImageBilateralBlurFilter()
    ...
    private fun createBlendFilter(
        ...
        return try {
            filterClass.newInstance().apply {
                bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.ic_switch_camera)
            }
        } catch (e: Exception) {
            e.printStackTrace()
            GPUImageFilter()
